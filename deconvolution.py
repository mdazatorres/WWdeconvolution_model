...
def shift_and_pad(ser: pd.Series, delay_distrb: np.ndarray) -> pd.Series:
    ...
    #ser_shifted= ser_extended# modificacion
...
ser = pd.Series([1, 2,3,4,5,6], index=pd.date_range(start='2020-04-01', periods=6))

delay_distrb = np.array([0.01,0.09, 0.2, 0.3, 0.4])

ser_deconvolved = deconvolve_series(ser, delay_distrb)

#assertTrue(ser_deconvolved.index.equals(pd.date_range(start='2020-03-30', periods=4)))
#assert_array_almost_equal(np.array([4 / 5, 32 / 35, 72 / 35, 16 / 7]),ser_deconvolved.values)
# def Conv(b):
#     convo = np.zeros(self.n_c - self.m + 1)  # np.zeros(self.n_c-self.m)
#     weight = gamma.sf(self.time, self.a, scale=1 / b)
#     weight = weight / np.sum(weight)
#     for i in range(self.n_c - self.m + 1):  # (self.n_c-self.m)
#         convo[i] = np.dot(self.cases[i:(self.m + i)], weight)
#     return (convo)